...
void gpu_matmul_double_works_correctly() {
    matrix2D_gpu_matmul(A_double, B_double, C_double);
...
void gpu_matmul_float_works_correctly() {
    matrix2D_gpu_matmul(A_float, B_float, C_float);
    TEST_ASSERT_FLOAT_ARRAY_WITHIN(.0001, C_data_float, C_float->data, C_float->y_length * C_float->x_length);
}

void gpu_matmul_works_on_matrices_not_multiples_of_8() {
    int size = 10;
    ...
    matrix2D_gpu_matmul(A_rect, B_rect, C_rect);
...
void register_gpu_matrix_multiply_tests() {
    ...
    RUN_TEST(gpu_matmul_works_on_matrices_not_multiples_of_8);
}
